...
void printPosOrder(bHeap h, int index)
{

   if(isBHeapLeaf(h,index))
   {
      printf("%d ",*(int*)h->data[index]);
      return;
   }
   
   printPosOrder(h, BHEAP_LEFTCHILD(index));
   
   if(BHEAP_RIGHTCHILD(index) < h->currentSize)
      printPosOrder(h, BHEAP_RIGHTCHILD(index));
